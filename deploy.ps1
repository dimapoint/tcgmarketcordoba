...
if (-not (Test-Path .env)) {
    throw "Falta .env en la raíz (se necesita al menos GOOGLE_CLIENT_ID opcional y se reescribe API_URL para el build)"
}
...
try {
    ...
    # Guard: el bundle debe incluir el entrypoint y el .env con la API_URL de prod.
    if (-not (Test-Path build/web/main.dart.js)) {
        throw "build/web/main.dart.js no existe — el build web quedó incompleto; abortando deploy"
    }
